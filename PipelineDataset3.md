...
**Tables**
- `silver.epa_vehicles`  
  <sub>1 row per vehicle_id</sub>
- `silver.epa_emissions`  
  <sub>Multiple rows per vehicle_id, keyed by sales area + standard</sub>
...
> ⛓️ *Dependencies are enforced in ADF so Gold runs only after Silver succeeds.*