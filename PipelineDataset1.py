...
# 1️⃣ Model Years Ingestion (bronze.model_years)
#    • Frequency: 🗓️ Once per year (beginning of year)
#    • Rationale: New vehicle model years are introduced annually.
#                 No value in more frequent runs.
#                 Annual execution = minimal cost & overhead.

# 2️⃣ Vehicle Catalog (Makes & Models) – Bronze Layer
#    • Tables: bronze.vehicle_makes, bronze.vehicle_models
#    • Frequency: 🔁 Weekly
#    • Rationale: Catalog evolves over time for current model year.
#                 Weekly refresh = catalog consistency, low API load.
...
#    • Rationale: Complaints are submitted continuously.
#                 Daily ingestion = up-to-date analytics.
#                 Merge logic ensures idempotency (no duplicates).
...
#    • Rationale: Gold tables are derived from Silver.
#                 Overwrite mode ensures consistency.
#                 Designed for reporting & dashboards.
